...
type PostCardPropTypes = ComponentProps<"div"> & {
	post: PostType;
};

export function PostCard({ post, ...props }: PostCardPropTypes) {
...
export function PostCardFull({ post, ...props }: PostCardPropTypes) {
	const [CommentsBoard, setCommentsBoards] = useState<null | boolean>(null);
	const CommentsBoardRef = useRef<null | HTMLDivElement>(null);
	const [comments, setComments] = useState<CommentType[] | null>(null);
	const [NumberOfComments] = useState<number>(10);
	...
	return (
		<div
			className="w-full bg-card rounded-2xl border-2 border-border p-4 sm:p-5 "
			onDoubleClick={() => {
				router.push(`/post/${post.id}`);
			}}
			{...props}
